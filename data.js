...
const EXPERIENCE = [
  {
    ...
    bullets: [
      ...
      'Engineered a <strong>slide-generation agent</strong> in which the LLM emits <strong>schema-validated actions</strong>, executed by a python-pptx backend — making hallucinated edits grammatically impossible. Added an event-sourced version tree with replay-based undo, a self-correcting editor, and a closed <strong>QA&rarr;refiner</strong> loop that auto-fixes overflow/overlap, all scored by a describe-then-judge <strong>LLM-as-judge</strong> eval. Next.js/React UI with a ReactFlow version tree.',
    ],
